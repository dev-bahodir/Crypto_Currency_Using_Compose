...
import dev.bahodir.cryptocurrencyapp.domain.use_case.get_coin.GetCoinUseCase
import dev.bahodir.cryptocurrencyapp.domain.use_case.get_coins.GetCoinsUseCase
    ...
    private fun getCoin(coinId: String) {
        getCoinUseCase(coinId).onEach {
            when(it) {
                ...
                is Resource.Error -> {
                    _state.value = CoinDetailState(error = it.message ?: "An expected error occurred")
                }
                is Resource.Loading -> {
